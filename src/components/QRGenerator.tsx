...
export default function QRGenerator() {
  ...
  useEffect(() => {
    ...
    if (url.length > 3) {
      QRCode.toCanvas(canvasRef.current, redirectUrl || url, { width: 220, margin: 2, color: { dark: '#0058c3', light: '#ffffff' } })
    } else {
      ...
      ctx.fillText('Enter URL to generate', 110, 103)
      ctx.fillText('your QR code', 110, 121)
    }
  }, [url, redirectUrl])
  ...
  const DownloadSection = () => (
    <div style={{ width: '100%' }}>
      {url.length > 3 && !shortId && (
        <button onClick={saveQR} disabled={saving} style={{ width: '100%', background: saving ? '#718096' : 'rgba(0,88,195,0.08)', color: saving ? '#fff' : '#0058c3', border: '1px solid rgba(0,88,195,0.2)', borderRadius: 4, padding: '12px', fontFamily: 'inherit', fontSize: 14, fontWeight: 600, cursor: 'pointer', marginBottom: 8, display: 'block' }}>
          {saving ? 'Saving...' : '✦ Make Dynamic (Free)'}
        </button>
      )}
      <button onClick={downloadPNG} style={{ width: '100%', background: 'linear-gradient(135deg,#0058c3,#0070f3)', color: '#fff', border: 'none', borderRadius: 4, padding: '13px', fontFamily: 'inherit', fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 8 }}>↓ Download PNG</button>
  ...
  return (
    <div style={{ fontFamily: 'Inter, system-ui, sans-serif', background: '#f7fafc', minHeight: '100vh' }}>
      <style>{`
        ...
        .main-layout { display: grid; grid-template-columns: 160px 1fr 280px; gap: 0; max-width: 1180px; margin: 0 auto; padding: 16px; align-items: start; }
        .analytics-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; }
        ...
        @media (max-width: 767px) {
          ...
          .main-layout { display: block !important; padding: 12px !important; }
          .analytics-grid { grid-template-columns: 1fr !important; }
          .meta-grid { grid-template-columns: 1fr !important; }
          .mobile-qr-section { display: block !important; }
        }
        .mobile-qr-section { display: none; }
      `}</style>
      ...
      <nav style={{ background: '#fff', borderBottom: '1px solid rgba(74,85,104,0.15)', padding: '0 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button className="hamburger-btn" onClick={() => setMenuOpen(!menuOpen)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 8, flexDirection: 'column', gap: 4 }}>
            <span style={{ display: 'block', width: 20, height: 2, background: '#181c1e', borderRadius: 1 }}></span>
            <span style={{ display: 'block', width: 20, height: 2, background: '#181c1e', borderRadius: 1 }}></span>
            <span style={{ display: 'block', width: 20, height: 2, background: '#181c1e', borderRadius: 1 }}></span>
          </button>
          <button className="desktop-nav-links" style={{ background: 'linear-gradient(135deg,#0058c3,#0070f3)', color: '#fff', border: 'none', borderRadius: 4, padding: '8px 14px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Go Premium →</button>
        </div>
      </nav>
      ...
      {menuOpen && (
        <div style={{ background: '#fff', borderBottom: '1px solid rgba(74,85,104,0.15)', padding: '8px 16px' }}>
          {['Generator','Analytics','Templates','API'].map((t,i) => (
            <div key={t} style={{ padding: '12px 0', fontSize: 14, fontWeight: 500, color: i === 0 ? '#0058c3' : '#4a5568', borderBottom: '1px solid rgba(74,85,104,0.06)', cursor: 'pointer' }}>{t}</div>
          ))}
        </div>
      )}
      ...
      <div className="main-layout">
        <aside className="desktop-sidebar" style={{ flexDirection: 'column', paddingRight: 12 }}>
        ...
        <main style={{ minWidth: 0, padding: '0 8px' }}>
          ...
          <div style={{ marginBottom: 10 }}>
            <input value={url} onChange={e => { setUrl(e.target.value); setShortId(''); setRedirectUrl('') }} type="url" placeholder="https://your-website.com" style={{ width: '100%', background: '#fff', border: '1px solid rgba(74,85,104,0.15)', borderRadius: 4, padding: '11px 14px', fontFamily: 'inherit', fontSize: 14, color: '#181c1e', outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <div style={{ marginBottom: 12 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'rgba(0,88,195,0.08)', color: '#0058c3', padding: '4px 10px', borderRadius: 2, fontSize: 10, fontWeight: 700, letterSpacing: '0.06em' }}>✦ SAFE-SCAN PROTECTION ACTIVE</div>
          </div>
          ...
          <div className="mobile-qr-section" style={{ background: '#fff', borderRadius: 8, padding: 16, marginBottom: 12, boxShadow: '0px 4px 16px rgba(24,28,30,0.06)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
        ...
        <aside className="desktop-right" style={{ flexDirection: 'column', paddingLeft: 12 }}>
          <div style={{ background: '#fff', borderRadius: 8, padding: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', boxShadow: '0px 8px 24px rgba(24,28,30,0.06)', gap: 12 }}>
            <canvas ref={canvasRef} width={220} height={220} style={{ borderRadius: 8, maxWidth: '100%' }} />
            <DownloadSection />
          </div>
        </aside>
