...
export default function QRGenerator() {
  ...
  }, [url, redirectUrl, isMobile, qrColor, bgColor])
  ...
  const ToolPanel = () => (
    <div style={{ background: '#f1f4f6', borderRadius: 8, padding: 14, marginBottom: 12 }}>
      {activeTool === 'Colors' && (
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#181c1e', marginBottom: 10 }}>QR Color</div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
            {['#0058c3','#181c1e','#16a34a','#dc2626','#7c3aed','#ea580c'].map(c => (
              <div key={c} onClick={() => setQrColor(c)} style={{ width: 28, height: 28, borderRadius: 4, background: c, cursor: 'pointer', border: qrColor === c ? '2px solid #0058c3' : '2px solid transparent', outline: qrColor === c ? '2px solid white' : 'none' }} />
            ))}
          </div>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#181c1e', marginBottom: 10 }}>Background</div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {['#ffffff','#f1f4f6','#fff7ed','#f0fdf4','#fef2f2','#181c1e'].map(c => (
              <div key={c} onClick={() => setBgColor(c)} style={{ width: 28, height: 28, borderRadius: 4, background: c, cursor: 'pointer', border: bgColor === c ? '2px solid #0058c3' : '1px solid rgba(74,85,104,0.2)', outline: bgColor === c ? '2px solid white' : 'none' }} />
            ))}
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {['No Frame','Simple Border','Rounded','Scan Me'].map(f => (
              <div key={f} style={{ background: '#fff', borderRadius: 6, padding: '8px 10px', fontSize: 11, fontWeight: 500, color: '#4a5568', cursor: 'pointer', border: '1px solid rgba(74,85,104,0.15)', textAlign: 'center' }}>{f}</div>
            ))}
          </div>
        </div>
      )}
      ...
      {activeTool === 'Customization' && (
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#181c1e', marginBottom: 8 }}>Quick Customize</div>
          <div style={{ fontSize: 11, color: '#718096', lineHeight: 1.5 }}>Select Colors, Frames, Logo, or Shapes from the menu to customize your QR code appearance.</div>
        </div>
      )}
    </div>
  )
  ...
  const AdBanner = () => (
    <div style={{ background: '#e5e9eb', borderRadius: 8, height: 72, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#718096', fontSize: 11, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', margin: '12px 0' }}>
      Advertisement
    </div>
  )

  const QRPreviewPanel = () => (
    <div style={{ background: '#fff', borderRadius: 8, padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, boxShadow: '0px 8px 24px rgba(24,28,30,0.06)', width: '100%' }}>
    ...
    <div style={{ fontFamily: 'Inter, system-ui, sans-serif', background: '#f7fafc', minHeight: '100vh' }}>
      <nav style={{ background: '#fff', borderBottom: '1px solid rgba(74,85,104,0.15)', padding: '0 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        {!isMobile && (
          <div style={{ display: 'flex', gap: 4 }}>
            <Link href="/" style={{ padding: '6px 12px', borderRadius: 4, fontSize: 13, fontWeight: 500, color: '#0058c3', border: 'none', background: 'none', cursor: 'pointer', borderBottom: '2px solid #0058c3', textDecoration: 'none' }}>Generator</Link>
            <Link href="/dashboard" style={{ padding: '6px 12px', borderRadius: 4, fontSize: 13, fontWeight: 500, color: '#4a5568', border: 'none', background: 'none', cursor: 'pointer', borderBottom: '2px solid transparent', textDecoration: 'none' }}>Analytics</Link>
            <Link href="/qr-code-for-restaurants" style={{ padding: '6px 12px', borderRadius: 4, fontSize: 13, fontWeight: 500, color: '#4a5568', border: 'none', background: 'none', cursor: 'pointer', borderBottom: '2px solid transparent', textDecoration: 'none' }}>Templates</Link>
            <a href="mailto:support@klickifyagency.com" style={{ padding: '6px 12px', borderRadius: 4, fontSize: 13, fontWeight: 500, color: '#4a5568', border: 'none', background: 'none', cursor: 'pointer', borderBottom: '2px solid transparent', textDecoration: 'none' }}>API</a>
          </div>
        )}
      ...
      {isMobile ? (
        <div style={{ padding: '0 16px 40px', maxWidth: 600, margin: '0 auto' }}>
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#718096', marginBottom: 8, paddingLeft: 2 }}>Designer Tools</div>
            <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 4 }}>
              {SIDEBAR_TOOLS.map(tool => (
                <button key={tool} onClick={() => { setActiveTool(tool); setShowToolPanel(true) }} style={{ background: activeTool === tool && showToolPanel ? '#0058c3' : '#fff', color: activeTool === tool && showToolPanel ? '#fff' : '#4a5568', border: '1px solid rgba(74,85,104,0.15)', borderRadius: 4, padding: '6px 12px', fontSize: 12, fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>{tool}</button>
              ))}
            </div>
          </div>
