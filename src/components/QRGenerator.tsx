    ...
    <div style={{ fontFamily: 'Inter, system-ui, sans-serif', background: '#f7fafc', minHeight: '100vh' }}>
      <style>{`
        @media (max-width: 767px) {
          ...
          .right-col { padding-left: 0 !important; width: 100% !important; }
        }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {qrCount > 0 && (
            <a href="/dashboard" style={{ display: 'flex', alignItems: 'center', gap: 5, background: '#f1f4f6', border: '1px solid rgba(74,85,104,0.15)', borderRadius: 4, padding: '5px 10px', fontSize: 12, fontWeight: 600, color: '#4a5568', textDecoration: 'none' }}>
              <span className="desktop-only">My QRs</span>
              <span style={{ background: '#0058c3', color: '#fff', borderRadius: 10, padding: '1px 5px', fontSize: 10 }}>{qrCount}</span>
            </a>
          )}
          <button className="hamburger" onClick={() => setMenuOpen(!menuOpen)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={{ display: 'block', width: 20, height: 2, background: '#181c1e', borderRadius: 1 }}></span>
            <span style={{ display: 'block', width: 20, height: 2, background: '#181c1e', borderRadius: 1 }}></span>
            <span style={{ display: 'block', width: 20, height: 2, background: '#181c1e', borderRadius: 1 }}></span>
          </button>
          <button className="desktop-only" style={{ background: 'linear-gradient(135deg,#0058c3,#0070f3)', color: '#fff', border: 'none', borderRadius: 4, padding: '8px 14px', fontSize: 13, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>Go Premium →</button>
        </div>
      </nav>

      {menuOpen && (
        <div className="mobile-only" style={{ background: '#fff', borderBottom: '1px solid rgba(74,85,104,0.15)', padding: '8px 16px' }}>
          {['Generator','Analytics','Templates','API'].map((t,i) => (
            <div key={t} style={{ padding: '10px 0', fontSize: 14, fontWeight: 500, color: i === 0 ? '#0058c3' : '#4a5568', borderBottom: '1px solid rgba(74,85,104,0.08)', cursor: 'pointer' }}>{t}</div>
          ))}
        </div>
      )}
      ...
      <div style={{ background: '#f1f4f6', padding: '8px 16px', textAlign: 'center', fontSize: 11, fontWeight: 500, color: '#4a5568', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
        ...
        <span style={{ background: 'rgba(0,88,195,0.08)', color: '#0058c3', padding: '1px 6px', borderRadius: 2, fontSize: 10, fontWeight: 700, letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>✦ TRULY FREE</span>
        <span style={{ fontSize: 11 }}>Dynamic QR — No Expiration, No Account.</span>
      </div>

      <div className="main-grid" style={{ display: 'grid', gridTemplateColumns: '168px 1fr 300px', gap: 0, maxWidth: 1180, margin: '0 auto', padding: '16px', alignItems: 'start' }}>

        <aside className="sidebar-col" style={{ paddingRight: 12 }}>
        ...
        <main style={{ padding: '0 8px', minWidth: 0 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'rgba(0,88,195,0.08)', color: '#0058c3', padding: '4px 10px', borderRadius: 2, fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', cursor: 'pointer' }}>✦ SAFE-SCAN PROTECTION ACTIVE</div>
          </div>

          <div style={{ background: '#f1f4f6', borderRadius: 8, padding: '12px 14px', display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#181c1e', marginBottom: 2, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                Dynamic Tracking
            ...
            <div onClick={() => setDynamic(!dynamic)} style={{ position: 'relative', width: 40, height: 22, flexShrink: 0, marginTop: 1, cursor: 'pointer' }}>
          ...
          <div className="mobile-only" style={{ marginBottom: 16 }}>
            <canvas ref={canvasRef} width={200} height={200} style={{ borderRadius: 8, display: 'block', margin: '0 auto 12px', maxWidth: '100%' }} />
            {url.length > 3 && !shortId && (
              <button onClick={saveQR} disabled={saving} style={{ width: '100%', background: saving ? '#718096' : 'rgba(0,88,195,0.08)', color: saving ? '#fff' : '#0058c3', border: '1px solid rgba(0,88,195,0.2)', borderRadius: 4, padding: '12px', fontFamily: 'inherit', fontSize: 14, fontWeight: 600, cursor: 'pointer', marginBottom: 8 }}>
                {saving ? 'Saving...' : '✦ Make Dynamic (Free)'}
              </button>
            )}
            <button onClick={downloadPNG} style={{ width: '100%', background: 'linear-gradient(135deg,#0058c3,#0070f3)', color: '#fff', border: 'none', borderRadius: 4, padding: '14px', fontFamily: 'inherit', fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 8 }}>↓ Download PNG</button>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <button onClick={downloadSVG} style={{ background: '#f1f4f6', border: '1px solid rgba(74,85,104,0.15)', borderRadius: 4, padding: '10px', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, color: '#4a5568', cursor: 'pointer' }}>SVG Vector</button>
              <button style={{ background: '#f1f4f6', border: '1px solid rgba(74,85,104,0.15)', borderRadius: 4, padding: '10px', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, color: '#4a5568', cursor: 'pointer' }}>Print PDF</button>
            </div>
          </div>
          ...
          <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid rgba(74,85,104,0.15)' }}>
            ...
            <div className="mobile-stack" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
              <div style={{ background: '#f1f4f6', borderRadius: 8, padding: 14 }}>
                ...
                {[['United States','42%',42],['Germany','28%',28],['Japan','15%',15],['Brazil','9%',9]].map(([c,p,w]) => (
                  <div key={String(c)}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 3 }}><span style={{ fontWeight: 500, fontSize: 11 }}>{c}</span><span style={{ fontWeight: 600, color: '#4a5568', fontSize: 11 }}>{p}</span></div>
              ...
              <div style={{ background: '#f1f4f6', borderRadius: 8, padding: 14, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#718096', marginBottom: 10, alignSelf: 'flex-start' }}>Device</div>
                ...
                <div style={{ display: 'flex', gap: 8, fontSize: 10, marginTop: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 3, color: '#4a5568' }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: '#0070f3', display: 'inline-block' }} />Mobile</span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 3, color: '#4a5568' }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: '#b5d4f4', display: 'inline-block' }} />Desktop</span>
                </div>
              </div>
              <div style={{ background: '#f1f4f6', borderRadius: 8, padding: 14 }}>
                <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#718096', marginBottom: 10 }}>Activity 24h</div>
        ...
        <aside className="right-col desktop-only" style={{ paddingLeft: 12 }}>
          <div style={{ background: '#fff', borderRadius: 8, padding: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', boxShadow: '0px 8px 24px rgba(24,28,30,0.06)' }}>
            <canvas ref={canvasRef} width={200} height={200} style={{ borderRadius: 8, marginBottom: 14, maxWidth: '100%' }} />
            {url.length > 3 && !shortId && (
              <button onClick={saveQR} disabled={saving} style={{ width: '100%', background: saving ? '#718096' : 'rgba(0,88,195,0.08)', color: saving ? '#fff' : '#0058c3', border: '1px solid rgba(0,88,195,0.2)', borderRadius: 4, padding: '10px', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, cursor: 'pointer', marginBottom: 8 }}>
                {saving ? 'Saving...' : '✦ Make Dynamic (Free)'}
              </button>
            )}
            <button onClick={downloadPNG} style={{ width: '100%', background: 'linear-gradient(135deg,#0058c3,#0070f3)', color: '#fff', border: 'none', borderRadius: 4, padding: '12px', fontFamily: 'inherit', fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 8 }}>↓ Download PNG</button>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, width: '100%', marginBottom: 8 }}>
              <button onClick={downloadSVG} style={{ background: '#f1f4f6', border: '1px solid rgba(74,85,104,0.15)', borderRadius: 4, padding: '8px', fontFamily: 'inherit', fontSize: 11, fontWeight: 600, color: '#4a5568', cursor: 'pointer' }}>SVG Vector</button>
              <button style={{ background: '#f1f4f6', border: '1px solid rgba(74,85,104,0.15)', borderRadius: 4, padding: '8px', fontFamily: 'inherit', fontSize: 11, fontWeight: 600, color: '#4a5568', cursor: 'pointer' }}>Print PDF</button>
            </div>
