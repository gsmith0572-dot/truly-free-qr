...
const PAGE_DATA: Record<string, Omit<NichePageProps, 'slug'>> = {
  ...
  'permanent-qr-code-free': {
    ...
    faqs: [
      ...
      { q: 'Is there any catch or hidden cost I should know about?', a: 'None. The business model is Google AdSense advertising displayed on trulyfreeqr.com. Users are never charged, never enrolled in subscriptions, and codes are never threatened with deactivation. The ads you see on the site are the complete and only revenue mechanism. Your codes are the product we protect, not the product we charge for.' },
  ...
  'qr-code-no-subscription': {
    ...
    faqs: [
      { q: 'Why do most QR generators require a subscription?', a: 'Dynamic QR codes require server infrastructure: a database to store redirect mappings, a server to process incoming scan requests, and bandwidth to handle traffic. Most companies monetize this infrastructure through annual subscriptions. Truly Free QR monetizes through Google AdSense advertising displayed on the website — the same infrastructure is funded by ad revenue, eliminating the need to charge users.' },
